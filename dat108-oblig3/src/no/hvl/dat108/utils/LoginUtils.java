...
public class LoginUtils {
	/**
	 * Klasse for Â bestemme overskrift pÂ inlogging
	 * 
	...
	public static String loginOverskrift(HttpServletRequest request) {
		...
		if(feilPassord != null){
			//Hvis bruker ble redirected tilbake p√• grunn av feil passord
			beOmPassord = "Feil brukernavn/passord. Pr√∏v igjen";
		}
		
		String kreverLogin = request.getParameter("trengerLogin");
		if(kreverLogin != null) {
			//Hvis ble redirected tilbake p√• grunn av session timeout
 ...
 */
	...
	public static boolean loggInn(HttpServletRequest request,Deltaker deltaker, int timeout){
		if(!LoginUtils.loginOk(request, deltaker)) {
			...
			//Fors√∏k √• hente session - hvis den ikke finnes, ikke opprett ny
	...
	/**
	 * Metode for Â starte en sesjon med deltaker
	 * 
	 * @param request fra servlet
	 * @param deltaker som skal logges inn
